...
export type Trip = {
  ...
  isYours: boolean;
  /** Everybody on it confirmed it, and it is therefore frozen. */
  confirmed: boolean;
  /** You are one of the people this trip charges. */
  youAreOnIt: boolean;
...
export async function listOpenTrips(carId: string): Promise<Trip[]> {
  ...
  const { data, error } = await supabase
    .from("trips")
    .select(
      `id, start_km, end_km, distance_km, driven_on, note, recorded_by, confirmed,
  ...
  return (data ?? []).map((row) => {
    ...
    return {
      ...
      youAreOnIt: participants.some((person) => person.isYou),
      participants,
